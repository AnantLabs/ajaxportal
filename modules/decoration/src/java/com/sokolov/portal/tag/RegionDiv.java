...
/**
 * Region tag for Ajax portal. It's based on &lt;DIV&gt; HTML tag.
 *
 * @author Sergei Sokolov
 * @version 1.0
 */
public class RegionDiv extends AbstractPortalTag {

    /** Holds CSS class for NoDecoration state */
    public final static String NODECORATION = "nodecoration";
    ...
    public final static String ACCORDION = "accordion";
    ...
    protected void correctState() {
        if ((getState() == null) || ("".equals(getState()))) {
            setState(STANDARD);
        } else if (!(NODECORATION.equals(getState().trim())
                      || WINDOW.equals(getState().trim())
                      || STANDARD.equals(getState().trim())
                      || ACCORDION.equals(getState().trim())
                      || TABS.equals(getState().trim())
                  )) {
            setState(STANDARD);
        }
    }
     ...
     *
     * @param width a minimal width of Region
     ...
     *
     * @param dropTarget a new value of editable property
     */
