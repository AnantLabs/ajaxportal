...
/**
 * Checks: is region  Drop Target or not.
 *
 * @param {string} region id
 * @return {boolean} if true the region is Drop Target.
 */
...
/**
 * Fill portlet listbox.
 *
 * @return {void}
 */
function showPortlet() {
    var portletListbox = $("portlets");
    for (var i = 0; i < portletListbox.options.length; i++) {
        if (portletListbox.options[i].selected) {
            var id = portletListbox.options[i].value;
    ...
    if (portletListbox.options.length == 0) { cleanPortlet(); }
}

/**
 * Save the current portlet.
 *
 * @return {void}
 */
function savePortlet() {
    ...
    if (window.portal.onChange) {                             
        try {
            eval("window.portal.onChange()")   // TODO: check
        } catch (e) {
...
/**
 * Clean the controls of the portlet properties.
 *
 * @return {void}
 */
 ...
 */
function newPortlet() {
    var regionListbox = $("regions");
    for (var i = 0; i < regionListbox.options.length; i++) {
        if (regionListbox.options[i].selected) {
            var id = regionListbox.options[i].value;
            ...
            if (portletRegion) {
                ...
                portletListbox.options.add(option);
    ...
    for (var i = 0; i < regionListbox.options.length; i++) {
        if (regionListbox.options[i].selected) {
            ...
            if (portletRegion) {
                ...
                var portletListbox = $("portlets");
                portletListbox.options.length = 0;
                ...
                //   if (portletListbox.options.length > 0) { portletListbox.options[0].selected = true; }
                showPortlet(portletListbox);

            }
        }
    }
    if (regionListbox.options.length == 0) { cleanRegion(); }
}
...
/**
 * Clean the controls of the region properties.
 *
 * @return {void}
 */
...
function showOptionsDialog() {
    ...
    obj.title='Portal page configuration';
    obj.innerHTML = html;
